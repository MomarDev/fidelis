...
public class AddServiceFormController implements Initializable {
    ...
    private void handlerButtonAction(ActionEvent event) {
        if (event.getSource() == btn_quitter) {
            ...
            if (listTypePrestation == null || listCategoriesPrestation == null || listSousCategories == null) {
                return;
            }
            ...
            if (result.get() == ButtonType.OK) {
                if (formIsOK()) {
                    Prestation ps = new Prestation(0, prestation, prix, sousCat);
                    ...
                    if (Utilis.driver.insertPrestation(ps)) {
                        Alert info = new Alert(Alert.AlertType.INFORMATION, "La prestation a été enregistrée avec succés.", ButtonType.OK);
                        info.show();
                        cat = null;
                        sousCat = null;
                        prestation = "";
                        ...
                        Alert erroSql = new Alert(Alert.AlertType.ERROR, "Une erreur s'est produite lors de l'enregistrement de la prestation", ButtonType.OK);
                        erroSql.show();
                    }
                } else {
                    Alert erreForm = new Alert(Alert.AlertType.ERROR, "Veuillez revérifier les informations saisies SVP.", ButtonType.OK);
                    erreForm.show();
                }
            } else {
                alert.close();
