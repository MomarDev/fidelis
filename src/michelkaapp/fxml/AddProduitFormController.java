        ...
        if (event.getSource() == btn_quitter) {
            ...
            if (formIsOk()) {
                if (Utilis.driver.existProduit(code, designation, false)) {
                    Alert existAlert = new Alert(Alert.AlertType.ERROR, "Le code barres ou la désignation est déjâ  utilisé sur un autre produit ", ButtonType.OK);
                    existAlert.show();
                    return;
                }
                ...
                if (result.get() == ButtonType.OK) {
                    if (Utilis.driver.insertProduit(produit)) {
                        prix = 0;
                        Alert Ainfo = new Alert(Alert.AlertType.INFORMATION, "Le produit a été bien enregistrer.", ButtonType.OK);
                        Ainfo.show();
